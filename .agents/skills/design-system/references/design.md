...
| **Status â€” pass** | `--color-status-pass` | `#2FA84F` | same |
| **Status â€” warn** | `--color-status-warn` | `#E8A33D` | same |
| **Status â€” fail** | `--color-status-fail` | `#D5473C` | same |
| **Status â€” idle** | `--color-status-idle` | `#9CA3AF` | same |

Because these live under `@theme inline` in Tailwind v4, they should already be usable as real utility classes: `bg-status-pass`, `text-status-pass`, `border-status-pass`, `bg-status-warn`, `text-status-warn`, `bg-status-fail`, `text-status-fail`, `bg-status-idle`, `text-status-idle` (plus opacity modifiers like `bg-status-pass/10`). **Verify this compiles** â€” if for any reason Tailwind doesn't pick them up, expose them explicitly with `@utility` in `index.css` rather than falling back to raw colors.
...
## Typography
- **Display** (`--font-display`: Space Grotesk) â€” page titles, hero headline. Not for body copy.
- **Sans** (`--font-sans`: Inter) â€” body text, descriptions, UI labels.
- **Mono** (`--font-mono`: IBM Plex Mono) â€” every piece of *data*: key codes, dB/percentage readouts, test IDs (`T-01`), status values, panel eyebrow labels (`LIVE OUTPUT`, `DIAGNOSTICS`, etc.). This is the single biggest signal that separates this product from a generic template â€” apply it consistently. `.spec-item` in `index.css` already encodes the correct style (mono, 0.65rem, 0.15em tracking, uppercase, muted color) for these labels â€” reuse that class instead of re-styling labels ad hoc per page.
...
- Shadows: `--shadow-sm/md/lg` in light mode; dark mode intentionally drops shadows to `none` and relies on borders instead â€” preserve that split.
- Motion keyframes already defined: `trace-draw` (signal line drawing in), `trace-pulse`, `trace-scroll`, `led-pulse` (status LED pulse). Reuse these rather than inventing new easing/animation per page. `prefers-reduced-motion` is already globally handled â€” don't bypass it in new components.

## Component conventions (read before touching any test page)

**1. Shared header/action bar.** Every test page hand-duplicates the same block: back arrow â†’ title â†’ description â†’ "Mark Issue"/"Mark Working" buttons. This should be one shared component (e.g. `TestPageHeader`) taking `testId`/`title`/`description` as props. Duplicating it 9 times is exactly why the status-color drift above happened â€” one file added extra hardcoded styling nobody else picked up.
...
**2. Hero state icon.** When a test is idle or awaiting permission, it shows a large icon inside a circle. Right now there are three different treatments in the codebase:
   - Camera / Microphone: `w-16 h-16 bg-primary/20 rounded-full` (64px, primary token) â€” **this is the correct pattern, standardize on it.**
...
**3. Panel labels.** Content panels get a small mono uppercase eyebrow label (`INTERACTIVE LAYOUT`, `LIVE OUTPUT`, `DIAGNOSTICS`, `DETECTED ACTIONS`, etc.) using `.spec-item`. Camera and Microphone's main preview panel is missing this label â€” every other panel on every other page has one.
